//
//  IngredientChoiceEntriesView.swift
...
struct IngredientChoiceEntriesView: View {
    ...
    var body: some View {
        VStack {
            ...
            TabView(selection: $index) {
                IngredientStockChoiceView(stocks: stocks, index: $index).tag(0)
                ModifyStocksView(stocks: ListOfIngredientViewModel(stocks)).tag(1)
            }.tabViewStyle(PageTabViewStyle(indexDisplayMode: .never))
            
            Spacer(minLength: 0)
                .navigationBarTitleDisplayMode(.inline)
                .navigationTitle("Entrée de stock par ingrédients")
        }
    }
}

struct IngredientChoiceEntriesView_Previews: PreviewProvider {
    static var previews: some View {
        IngredientChoiceEntriesView()
    }
}
